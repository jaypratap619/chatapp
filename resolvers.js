...
const resolvers = {
    Query: {
        ...
        messagesByUser: async (__, { receiverId }, { userId }) => {
            if (!userId) throw new ForbiddenError("You must be logged in")
    ...
    Mutation: {
        ...
        signinUser: async (_, { userSignin }) => {
            ...
            if (!doMatch) throw new AuthenticationError("email or password invalid")
        ...
        createMessage: async (_, { receiverId, text }, { userId }) => {
            ...
            pubsub.publish(MESSAGE_ADDED, { messageAdded: message })     // firing the event
            return message
        }
    },
    Subscription: {
        messageAdded: {
            subscribe: () => pubsub.asyncIterator(MESSAGE_ADDED)      // this will listen to the event 
        }
    }
}
